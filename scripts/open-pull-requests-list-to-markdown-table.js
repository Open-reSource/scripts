// Usage: node scripts/open-pull-requests-list-to-markdown-table.js
// This script fetches all open pull requests from a GitHub repository and saves them to a Markdown file with a table.
...
const OUTPUT_FILE = `open_pull_requests_${GITHUB_ORG}_${GITHUB_REPOSITORY}.md`
...
// Function to write pull requests to a Markdown file with a table
function writeToMarkdownFile(pullRequests) {
  ...
  const markdownContent = [
    '| PR Number | Title | |',
    '| --- | --- | --- |',
    ...pullRequests.map(pr => `| [#${pr.number}](${pr.html_url}) | ${pr.title} | |`), // Format each row
  ].join('\n');
